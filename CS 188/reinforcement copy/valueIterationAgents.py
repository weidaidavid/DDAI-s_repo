...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        while iterated < self.iterations:
            temp_values = self.values.copy()
            states_to_iterate = self.mdp.getStates()
            
            for state in states_to_iterate:
                best_action = self.computeActionFromValues(state)
                if best_action != None:
                    corresponding_val = self.computeQValueFromValues(state,best_action)
                    
                    temp_values[state] = corresponding_val
    ...
    def computeQValueFromValues(self, state, action):
        ...
        summation = 0
        sPrimesNTs = self.mdp.getTransitionStatesAndProbs(state,action)
        for stata,prob  in sPrimesNTs:
            oneOfUs = prob * (self.mdp.getReward(state,action,stata) + self.discount * self.values[stata])
            summation += oneOfUs
        return  summation
    ...
    def computeActionFromValues(self, state):
        ...
        for action in actions:
            now = self.computeQValueFromValues(state,action)
            if now > maxim:
                maxim = now
                policy = action
        return policy
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        while iterated < self.iterations:
            ...
            for state in states_to_iterate:
                if iterated <  self.iterations:
                    best_action = self.computeActionFromValues(state)
                    if best_action != None:
                        corresponding_val = self.computeQValueFromValues(state,best_action)
                        self.values[state] = corresponding_val
                iterated += 1
