...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        _solnNumToPlace = new Place[_width * _height + 2];
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                int k = _solution[x][y];
                if (k == 1) {
                    Sq theSquare = new Sq(x, y, 1, true,
                            arrowDirection(x, y), -1);
                    _board[x][y] = theSquare;
                    _allSquares.add(theSquare);
                } else if (k == last) {
                    Sq theSquare = new Sq(x, y, last, true,
                            arrowDirection(x, y), -1);
                    _board[x][y] = theSquare;
                    _allSquares.add(theSquare);
                } else {
                    Sq theSquare = new Sq(x, y, 0, false,
                            arrowDirection(x, y), -1);
                    _board[x][y] = theSquare;
                    _allSquares.add(theSquare);
                }
        ...
        for (int checker = 1; checker <= last; checker++) {
            if (_solnNumToPlace[checker] == null) {
                throw new IllegalArgumentException("bad args bro");
            }
        }
        for (Sq square1 : _allSquares) {
            square1._successors = allSuccessors(square1.x,
                    square1.y, square1._dir);
            square1._predecessors = new PlaceList();
            for (int j = 0; j < _width; j++) {
                for (int i = 0; i < _height; i++) {
                    if (dirOf(j, i, square1.x, square1.y) > 0) {
                        square1._predecessors.add(pl(j, i));
                    }
                }
            }
        }
    ...
    Model(Model model) {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                Sq squareO = new Sq(model._board[x][y]);
                _board[x][y] = squareO;
                _allSquares.add(squareO);
            }
        }
        for (Sq square1 : _allSquares) {
            square1._successors = allSuccessors(square1.x,
    ...
    boolean autoconnect() {
        ...
        for (Sq sq : _allSquares) {
            for (Place sq2 : sq._successors) {
                if (_board[sq2.x][sq2.y]._sequenceNum - 1 == sq._sequenceNum) {
                    sq.connect(_board[sq2.x][sq2.y]);
                    yeah = true;
                }
            }
        }
    ...
    void solve() {
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                Sq squareDa = _board[x][y];
                squareDa._sequenceNum = _solution[x][y];
            }
        }
        autoconnect();
    }
    ...
    private int arrowDirection(int x, int y) {
        ...
        Place suma = solnNumToPlace(seq0);
        Place haha = solnNumToPlace(seq0 + 1);
        if (haha != null) {
            return dirOf(suma.x, suma.y, haha.x, haha.y);
        } else {
            return 0;
        }
    }
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            ...
            if (s1._predecessor != null || this._successor != null
                    || s1._head == this._head || s1._sequenceNum == 1
        ...
        boolean connect(Sq s1) {
            ...
            if (connectable(s1)) {
                ...
                if (this._sequenceNum != 0) {
                    int sequenceTemp = this._sequenceNum + 1;
                    Sq aThis = this;
                    releaseGroup(s1._group);
                    while (aThis._successor != null) {
                        aThis = aThis._successor;
                        aThis._head = this._head;
                        aThis._group = this._group;
                        aThis._sequenceNum = sequenceTemp;
                        sequenceTemp++;
                    }
                } else if (s1._sequenceNum != 0) {
                    s1._head = this._head;
                    Sq aThisA = this;
                    while (aThisA._successor != null) {
                        aThisA = aThisA._successor;
                        aThisA._head = this._head;
                    }
                    releaseGroup(this._group);
                    int sequenceTemp = s1._sequenceNum - 1;
                    Sq aThat = s1;
                    while (aThat._predecessor != null) {
                        aThat = aThat._predecessor;
                        aThat._group = s1._group;
                        aThat._sequenceNum = sequenceTemp;
                        sequenceTemp -= 1;
                    }
                } else {
                    int groupNum = joinGroups(this._group, s1._group);
                    Sq forwardTemp = this;
                    Sq backwardTemp = this;
                    this._group = groupNum;
                    while (forwardTemp._successor != null) {
                        forwardTemp = forwardTemp._successor;
                        forwardTemp._group = groupNum;
                        forwardTemp._head = this._head;
                    }
                    while (backwardTemp._predecessor != null) {
                        backwardTemp = backwardTemp._predecessor;
                        backwardTemp._group = groupNum;
                    }
                }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                ...
                Sq tP = this; int check = 0; int predONah = 0;
                while (tP != null) {
                    if (tP._hasFixedNum) {
                        check += 1;
                    }
                    predONah++; tP = tP._predecessor;
                }
                if (check == 0) {
                    this._sequenceNum = 0;
                    if (predONah > 1) {
                        tP = this; int abc = newGroup();
                        while (tP != null) {
                            tP._sequenceNum = 0; tP._group = abc;
                            tP = tP._predecessor;
                        }
                    } else {
                        this._group = -1;
                ...
                Sq nextPoint = next; int checkA = 0; int succOrNah = 0;
                while (nextPoint != null) {
                    if (nextPoint._hasFixedNum) {
                        checkA += 1;
                    }
                    succOrNah++; nextPoint = nextPoint._successor;
                }
                if (checkA == 0) {
                    next._sequenceNum = 0;
                    if (succOrNah > 1) {
            ...
            while (nextPointer != null) {
                nextPointer._head = next; nextPointer = nextPointer._successor;
            }
        }
        /** a helper to shorten disconnect.
         * @param thisA is  a square
         * @param  cd is  an  int
         * */
        public void helper(Sq thisA,  int cd) {
            while (thisA != null) {
                thisA._sequenceNum = 0; thisA._group = cd;
                thisA = thisA._successor;
            }
        }
        /** a helper to shorten disconnect. *
         * @param  david is  a  square
         */
        public void  helper1(Sq david) {
            if (this._predecessor == null
                    && david._successor == null) {
                releaseGroup(this._group); releaseGroup(david._group);
                david._head = david; this._group = david._group = -1;
