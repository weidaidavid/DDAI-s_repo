    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        if (start.sequenceNum() == 0) {
            ...
            for (Place p : start.successors()) {
                if (start.connectable(model.get(p.x, p.y))) {
                    abbbb += 1;
                }
            }
            if (abbbb == 1) {
            ...
            for (Place p : start.successors()) {
                if (start.connectable(model.get(p.x, p.y))) {
                    abba++;
                    int bbbb = model.get(p.x, p.y).sequenceNum();
                    int ccc = start.sequenceNum();
                    if (bbbb == ccc + 1) {
                        return model.get(p.x, p.y);
                    }
                }
            }
            if (abba == 1) {
                for (Place aha : start.successors()) {
                    if (start.connectable(model.get(aha.x, aha.y))) {
                        return model.get(aha.x, aha.y);
                    }
                }
            }
        }
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        if (end.sequenceNum() == 0) {
            ...
            for (Place p : end.predecessors()) {
                if (model.get(p.x, p.y).connectable(end)) {
                    abbbb += 1;
                }
            }
            if (abbbb == 1) {
            ...
            if (abba == 1) {
                for (Place aha : end.predecessors()) {
                    if (model.get(aha.x, aha.y).connectable(end)) {
                        return model.get(aha.x, aha.y);
                    }
                }
            }
